...
let rec divide k seznam =
	let rec divide' k acc seznam =
		match k, seznam with
		| k, _ when (k >= List.length seznam) -> (seznam, [])
		| k, _ when (k < 0) -> ([], seznam)
		| _, [] -> ([], [])
		| 0, xs -> (List.rev acc, xs)
		| k, x :: xs -> divide' (k-1) (x :: acc) xs
	in
	divide' k [] seznam
...
let insert x k seznam =
	let (levi, desni) = divide k seznam in
	levi @ [x] @ desni
...
let rotate n seznam =
	let (levi, desni) = divide n seznam in
	desni @ levi
...
let rec max_on_components prvi drugi =
...
let rec second_largest seznam =
	let rec najvecja_vrednost = function
		| [] -> failwith "Seznam je prekratek!"
		| [x] -> x
		| x :: xs -> max x (najvecja_vrednost xs)
	in
	najvecja_vrednost (remove (najvecja_vrednost seznam) seznam)