...
let euro_to_dollar euro =
  match euro with
  | Euro x -> Dollar (1.136822 *. x)


let dollar_to_euro dollar =
  match dollar with
  | Dollar x -> Euro (0.879645186 *. x)
...
let to_pound valuta =
  match valuta with
  | Yen x -> Pound (0.00707634287 *. x)
  | Pound x -> Pound x
  | Krona x -> Pound (0.0860585 *. x)
  | Frank x -> Pound (0.777685 *. x)
...
let rec intbool_map f_int f_bool = function
  ...
  | Bool_value (x, xs) -> Bool_value(f_bool x, intbool_map f_int f_bool xs)
...
let rec intbool_reverse seznam =
  let rec reverse' acc = function
    | Empty -> acc
    | Int_value (x, xs) -> reverse' (Int_value (x, acc)) xs
    | Bool_value (x, xs) -> reverse' (Bool_value (x, acc)) xs
  in
  reverse' Empty seznam
...
let rec intbool_separate seznam =
  let rec separate' stevila booli = function
    | Empty -> (List.rev stevila, List.rev booli)
    | Int_value (x, xs)-> separate' (x :: stevila) booli xs
    | Bool_value (x, xs) -> separate' stevila (x :: booli) xs
  in
  separate' [] [] seznam
...
let test = {name = "Andraž"; status = Student(Frost, 3)}
...
let rec count_magic seznam =
  ...
  in
  count' {fire = 0; frost = 0; arcane = 0} seznam
...
let rec find_candidate magic specialisation seznam =
  let stevec = 
    ...
    | Teacher -> 5
  in
  let rec find_candidate' magic stevec = function
    | [] -> None
    | {name; status} :: xs ->
      match status with
      | Newbie -> find_candidate' magic stevec xs
      | Employed (_, _) -> find_candidate' magic stevec xs
      | Student (magija, leta) -> 
        if magija = magic then
          if leta >= stevec then
            Some name
          else
            find_candidate' magic stevec xs
        else
          find_candidate' magic stevec xs
  in
  find_candidate' magic stevec seznam