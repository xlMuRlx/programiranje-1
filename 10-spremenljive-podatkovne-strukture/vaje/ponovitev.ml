...
let randlist len max =
  let rec randlist' len max acc = 
    match len with
    | 0 -> acc
    | k -> randlist' (k-1) max (Random.int max :: acc)
  in
  randlist' len max []
...
let test_sort_fun our_sort =
  let seznam = (randlist 100 100) in
  (our_sort seznam = List.sort compare seznam)
...
let rec min_and_rest seznam =
  ...
  | [] -> None
  | ys -> 
    let nov_min = min (List.hd seznam) seznam in
    Some(nov_min, remove_first nov_min seznam)
...
let rec selection_sort seznam =
  let rec sort' acc = function
    | [] -> List.rev acc
    | ys -> (
      match min_and_rest ys with
      | None -> failwith "Se ne zgodi!"
      | Some (min, rest) -> sort' (min :: acc) rest
    )
  in
  sort' [] seznam
